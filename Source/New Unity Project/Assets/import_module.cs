...
public void GenerateDataFrame ()
	{
		...
		sceneLight.transform.LookAt(loadedModel.transform.position);
		Vector3 obj_coords= mycam.WorldToScreenPoint(collider.bounds.center);
		float obj_x = obj_coords.x/mycam.pixelWidth;
		float obj_y=obj_coords.y/mycam.pixelHeight;
		float[] obj_wh = getWandH() ;
		...
		string dataLine = string.Format("{0}.png,{1},{2},{3},{4},{5},{6},{7}", i, obj_x,obj_y,obj_wh[0]/mycam.pixelWidth,obj_wh[1]/mycam.pixelHeight,rot_x,rot_y,rot_z);
